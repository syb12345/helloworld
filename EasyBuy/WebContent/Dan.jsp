...
<div id="header" class="wrap">
	...
	<div class="help"><c:if test="${name!=null}"><a href="selectdd?dd=${name.EU_USER_ID }">个人订单</a>
	</c:if><c:if test="${name!=null}">当前用户${name.EU_USER_ID }</c:if><a href="ShopSelect" class="shopping">购物车</a>
	<c:if test="${name==null}"><a href="login.jsp">登录</a>|<a href="register.jsp">注册</a>
	</c:if><c:if test="${name!=null}"><a href="zx">退出</a></c:if><a href="SelallServlet">留言</a>
	<c:if test="${name.EU_STATUS==2}"><a href="manage/index.jsp" >去后台</a></c:if></div>
	<div class="navbar">
		<ul class="clearfix">
			<c:choose>
			...
			<table border="1"  width="600px" height="100px" bordercolor="red">
				...
				<c:forEach var="d" items="${dan}">
					<tr align="center">
						...
						<td>${d.EP_STOCK }</td>
						<td id="evaluation${d }"><input type="radio" name="group${d }" />较差
						<input type="radio" name="group${d }" />一般
						<input type="radio" name="group${d }"  />较好
						<input type="radio" name="group${d }" } />极好<br></br>
						<input type="button" value="提交"  onclick="evaluate('evaluation${d}')"/></td>
					</tr>
					</c:forEach>
			</table>
			</c:if>
...
if(n!=null){
	document.write('<c:forEach var="d" items="${dan}">')
	document.getElementById("evaluation${d}").innerHTML="感谢您的评价！"
	document.write('</c:forEach>')
}
function evaluate(a){
	var evaluation=document.getElementById(a);
	evaluation.innerHTML="感谢您的评价！"
	sessionStorage.n='1';
}
</script>
</body>
